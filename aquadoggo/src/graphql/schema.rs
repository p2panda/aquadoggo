...
use async_graphql::{
    ContainerType, ContextSelectionSet, EmptySubscription, MergedObject,
    Name, OutputType, Positioned, Schema, SelectionField, ServerResult, Value,
};
...
pub type RootSchema = Schema<QueryRoot, Mutation, EmptySubscription>;
...
#[async_trait]
impl ContainerType for DocumentQuery {
...
impl OutputType for DocumentQuery {
    ...
    fn create_type_info(registry: &mut async_graphql::registry::Registry) -> String {
        ...
        registry.create_output_type::<DocumentQuery, _>(MetaTypeId::Object, |reg| {
            let schema: SchemaId =
...
#[derive(MergedObject)]
pub struct QueryRoot(Query, DocumentQuery);

impl QueryRoot {
    pub fn new(pool: Pool) -> Self {
        QueryRoot(Query::default(), DocumentQuery(pool))
    }
}

pub fn build_root_schema(pool: Pool) -> RootSchema {
    let query: QueryRoot = QueryRoot::new(pool.clone());
