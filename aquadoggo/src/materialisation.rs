...
pub async fn materialise(pool: &Pool, document_id: &Hash) -> Result<DocumentView> {
    ...
    log::debug!("Materialisation yields {:?}", document_view);
    ...
    if *document.schema()
        == Hash::new("0020c65567ae37efea293e34a9c7d13f8f2bf23dbdc3b5c7b9ab46293111c48fc78b")
            .unwrap()
    {
        write_document(pool, document_id, document_view).await?;
    }
    Ok(document_view.to_owned())
}
